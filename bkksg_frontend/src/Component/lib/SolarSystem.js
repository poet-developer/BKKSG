     const drawSolarSystemAnimation = (cv, ctx, ratio) => {
     
    //  cv = document.querySelector(".c");
    //  ctx = cv.getContext("2d");
      ...
      let timeId;
      function getPlanetPos(interval, initInfo) {
        let _Dot;
        if (typeof interval === "object") {
          ...
          let _cometDot = setPlanetPosToRandom(
            cometPos.x,
            cometPos.y,
            interval
          );
          ...
          for (var i = orbitIndex.first; i < orbitIndex.fin; i++) {
            if (i === 1) {
              planetPos.x[i] =
                centerPos.x -
                (interval * (i + 1)) / (initInfo[i - 1].orbit * 2);
              planetPos.y[i] =
                centerPos.y -
                (interval * (i + 1)) / (initInfo[i - 1].orbit * 2);
      ...
      function setPlanetPosToRandom(planetPosX, planetPosY, interval) {
        let randomOption = {
      ...
      function drawComet(ctx, position, dot, tailHeight, interval) {
        let _cometCenterPos = {
      ...
      function paintPlanet(initInfo, dotPos) {
      ...
      const SolarSystem = {
        ...
        cometOrbit: function (ctx, interval) {
          ctx.strokeRect(
      ...
      function setSatellitePosToRandom(initInfo) {
        let randomOption = {
          first: Math.random() < 0.5,
          second: Math.random() < 0.5,
        };
        ...
        if (randomOption.first) {
          if (randomOption.second) {
            initInfo.position = "top";
            // return {x: _posX,
            // y: parentPlanetPos.y-scaleRatio*(initInfo.orbit.min)/2 }
          } else {
            initInfo.position = "bottom";
            // return {x: _posX,
            // y: parentPlanetPos.y-scaleRatio*(initInfo.orbit.max)/2 }
          }
        } else {
          if (randomOption.second) {
            initInfo.position = "left";
            // return {x: parentPlanetPos.x-scaleRatio*(initInfo.orbit.min)/2,
            // y: _posY }
          } else {
            initInfo.position = "right";
            // return {x: parentPlanetPos.x-scaleRatio*(initInfo.orbit.max)/2,
            // y: _posY }
          }
        }
      }
      ...
      Satellite.prototype.catchPlanet = function () {
        ...
        switch (this.initInfo.position) {
          case "left":
            this.initInfo.orbit.y =
              this.initInfo.orbit.y + this.initInfo.speed.y;
            if (this.initInfo.orbit.y >= this.initInfo.orbit.max) {
              _satellitePos.y =
                this.parentPlanetPos.y -
                (scaleRatio * this.initInfo.orbit.y) / 2;
          ...
          case "bottom":
            this.initInfo.orbit.x =
              this.initInfo.orbit.x + this.initInfo.speed.x;

            if (this.initInfo.orbit.x >= this.initInfo.orbit.max) {
              _satellitePos.x =
                this.parentPlanetPos.x -
                (scaleRatio * this.initInfo.orbit.x) / 2;
      ...
      function makeComet(
        ...
        switch (initInfo.position) {
          case "left":
            dot.y = dot.y + initInfo.speed.y;
            if (dot.y >= cometOriginPos.y + orbitLength.y) {
              dot.y = cometOriginPos.y + orbitLength.y;
              initInfo.speed.x = speed;
              initInfo.speed.y = 0;
              initInfo.position = "bottom";
            }
            break;
          ...
          case "bottom":
            dot.x = dot.x + initInfo.speed.x;
            if (dot.x >= cometOriginPos.x + orbitLength.x) {
              dot.x = cometOriginPos.x + orbitLength.x;
              initInfo.speed.x = 0;
              initInfo.speed.y = speed;
              initInfo.position = "right";
            }
            break;
          ...
          case "right":
            dot.y = dot.y - initInfo.speed.y;
            if (dot.y <= cometOriginPos.y) {
              dot.y = cometOriginPos.y;
              initInfo.speed.x = speed;
              initInfo.speed.y = 0;
              initInfo.position = "top";
            }
          ...
          case "top":
            dot.x = dot.x - initInfo.speed.x;
            if (dot.x <= cometOriginPos.x) {
              dot.x = cometOriginPos.x;
              initInfo.speed.x = 0;
              initInfo.speed.y = speed;
              initInfo.position = "left";
            }
