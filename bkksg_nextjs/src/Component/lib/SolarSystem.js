...
const drawSolarSystemAnimation = (cv, ctx, orbitRatio, planetRatio) => {
  ...
  const setPlanetPosToRandom = (planetPosX, planetPosY, interval) => {
    ...
    if (typeof interval === "object") {
      ...
      if (randomOption.first) {
        ...
        else return { x: _posX, y: planetPosY + interval.y, corner: "bottom" };
      } else {
        if (randomOption.second)
          return { x: planetPosX, y: _posY, corner: "left" };
        else return { x: planetPosX + interval.x, y: _posY, corner: "right" };
      }
    } else {
      let _posX =
      ...
      if (randomOption.first) {
        if (randomOption.second)
          return { x: _posX, y: planetPosY, corner: "top" };
        else return { x: _posX, y: planetPosY + interval, corner: "bottom" };
      } else {
        if (randomOption.second)
          return { x: planetPosX, y: _posY, corner: "left" };
        else return { x: planetPosX + interval, y: _posY, corner: "right" };
      }
    }
  };
  ...
  Comet.prototype.makeComet = function () {
    drawComet(
      this.initInfo.position,
      this.dot,
      this.initInfo.tailHeight,
      this.squareInterval,
    );
    switch (this.initInfo.position) {
      case "left":
