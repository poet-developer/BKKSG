...
const LogoFooter = styled.div`
  border-top: ${(props) =>
    props.theme === true
      ? "solid 0.1px rgba(200,200,200,0.5)"
      : "solid 0.1px rgba(200, 200, 200, 0.5)"};
`;
  ...
  return (
    <div className="modal-container">
      <DetailHeader
        ...
        imgSrc={
          data.cover_src
            ? `url(${process.env.NEXT_PUBLIC_REACT_AWS_CLOUDFRONT}w1024/${data.cover_src})`
            : ""
        }
      >
        <div className="project-header">
          <Title topic={data.topic}>{data.title}</Title>
        </div>
      </DetailHeader>
      <MainContainer>
        <MainItem topic={data.topic}>{Parser(data.description)}</MainItem>
        <LogoFooter className="logo-footer" theme={themeMode}>
          비껴서기 | BKKSG
        </LogoFooter>
      </MainContainer>
    </div>
