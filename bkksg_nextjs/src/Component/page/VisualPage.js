...
const ImageContainer = styled.div`
  background-image: ${(props) =>
    props
      ? `url(${process.env.NEXT_PUBLIC_REACT_AWS_CLOUDFRONT}w1024/${props.src})`
      : ``};
`;
...
const VisualPage = (props) => {
  ...
  return (
    <div className="modal-container">
      <DetailSection className="visual-detail-section">
        ...
        <div className="visual-desc-container">
          <div className="visual-desc-item">
            ...
            />
            <LogoFooter className="logo-footer" themeMode={themeMode}>
              비껴서기 | BKKSG
            </LogoFooter>
          </div>
        </div>
      ...
      <VisualModal>
        <Cursor
          isCursor={data.link ? true : false}
          onClick={data.link ? goLink : null}
        />
        <ImageContainer
          className="img-container"
          src={data.cover_src ? data.cover_src : ""}
        />
        {/* 고화질 사진 불러옴 */}
