...
const Admin = ({ detailHandler }) => {
  ...
  if (content && page) {
    if (list === "") {
      contentList = content.slice(offset, offset + contentLimit).map((item) => {
        return (
      ...
      contentList = content
        .filter((item) => {
          if (item === "") return item;
            ...
            item.title.toLowerCase().includes(list.toLowerCase()) ||
            item.description.toLowerCase().includes(list.toLowerCase())
          )
            return item;
        })
        .map((item) => {
          return <TableContent data={item} read={readPreviewProcess} />;
        });
    }
  }
  /** Pagination */
  ...
  const createProcess = async () => {
    try {
      await axios
        ...
        .then(setMode(""))
        .then(alert("Uploaded!")); // 데이터 생성 후 바로 윈도우를 리로드 시켜 변경된 관리자 페이지를 확인
    } catch (err) {
      console.log(err);
      throw new Error(err);
    }
  ...
  return logined ? ( // admin 로그인 여부를 확인
    <>
      {/* Admin Container */}
          ...
          <div>
            ...
            <form
              ...
              onSubmit={(e) => {
                ...
                if (_confrimed)
                  deleteProcess(data.id, data.type, data.cover_src);
                else console.log("거부");
              }}
            >
